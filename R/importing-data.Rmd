...
Duas funções que às vezes são úteis ao baixar dados da internet são `tempdir` e `tempfile`. A primeira cria um diretório com um nome aleatório provavelmente único. Da mesma forma, `tempfile` cria uma cadeia de caracteres, não um arquivo, que provavelmente é um nome de arquivo exclusivo. Eles podem executar um comando, como o seguinte, que exclui o arquivo temporário depois que os dados são importados:
