...
Chamamos isso de **modelo de amostragem**, pois estamos modelando o comportamento aleatório da roleta com a amostragem das opções disponíveis. O ganho total de $S$ é simplesmente a soma desses 1.000 sorteios independentes:
...
Descrevemos anteriormente modelos de amostragem para sorteios. Agora, revisaremos a teoria matemática que nos permite aproximar as distribuições de probabilidade para a soma dos sorteios. Depois de fazer isso estaremos aptos para ajudar o cassino a prever quanto dinheiro ganharão. A mesma abordagem que usamos para a soma dos sorteios será útil para descrever a distribuição das médias e a proporção que precisaremos para entender como as pesquisas funcionam.
...
Uma variável aleatória irá variar em torno do valor esperado de uma maneira que, se você pegar a média de muitos, muitos sorteios, a média dos sorteios se aproximará do valor esperado, aproximando-se cada vez mais à medida que os sorteios aumentam.
...
Agora, a razão pela qual definimos o valor esperado é porque essa definição matemática é útil para aproximar as distribuições de probabilidade da soma, que é útil para descrever a distribuição de médias e proporções. O primeiro fato útil é que _o valor esperado da soma dos sorteios_ é:

$$
\mbox{}\mbox{número de sorteios } \times \mbox{ média dos números na urna}
$$
...
**Se nossos sorteios são independentes**, o erro padrão da soma é dado pela equação:

$$
\sqrt{\mbox{número de sorteios }} \times \mbox{ desvio padrão dos números na urna}
$$
...
O Teorema do Limite Central, ou CLT (do inglês _Central Limit Theorem_), nos diz que quando o número de sorteios, também chamado _tamanho da amostra_, é grande, a distribuição de probabilidade da soma de sorteios independentes é aproximadamente normal. Como modelos de amostragem são usados para muitos processos de geração de dados, o Teorema do Limite Central é considerado uma das ideias matemáticas mais importantes da história.
...
Usando o Teorema do Limite Central, podemos pular a simulação de Monte Carlo e, em vez disso, calcular a probabilidade de o cassino perder dinheiro usando esta aproximação:
...
O Teorema do Limite Central funciona quando o número de sorteios é amplo. Mas amplo é um termo relativo. Em muitas circunstâncias, apenas 30 sorteios são suficientes para que o Teorema do Limite Central seja útil. Em alguns casos específicos, apenas 10 sorteios são suficientes. No entanto, essas não devem ser consideradas regras gerais. Por exemplo, quando a probabilidade de sucesso é muito pequena, precisamos de tamanhos de amostra muito maiores.
...
Se $X$ são sorteios independentes das urnas, então todos têm o mesmo valor esperado. Vamos chamar isso de $\mu$. Logo:
...
Para ver por que isso é intuitivo, considere alterar as unidades. Se mudarmos as unidades de uma variável aleatória, digamos de dólares para centavos, a expectativa deve mudar da mesma maneira. Uma consequência dos dois fatos anteriores é que o valor esperado da média de sorteios independentes da mesma urna é o valor esperado da urna. Chame isso de $\mu$ novamente:
...
Uma consequência de 3 e 4 é que o erro padrão da média de sorteios independentes para a mesma urna é o desvio padrão da urna dividido pela raiz quadrada de $n$ (o número de sorteios), chame isso de $\sigma$:
...
1\. Na roleta americana, você também pode apostar no verde. Existem 18 vermelhos, 18 pretos e dois verdes (0 e 00). Quais são as chances de um green sair?
...
a. 1.000 simulações não são suficientes. Se fizermos mais, eles coincidem.
